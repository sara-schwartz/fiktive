...
| `lab_dm_forsker` | Laboratoriedatabasens Forskertabel | event_from_person | Lab test results per request. `analysiscode` needs a LabTerm/IFCC NPU catalogue (`FIKTIVE_LABTERM`); errors with a `SCHEMA GAP` otherwise rather than inventing codes. `value`/`unit`/reference interval are realistic for ~20 common analytes (potassium, sodium, creatinine, HbA1c, lipids, ...); any other NPU code still falls back to a bare placeholder value |
