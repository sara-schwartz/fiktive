...
in its own right, but especially for KKH/KKHNG, where a name like `g03011`
...
bundled metadata for **KKH** (Danish Diet, Cancer and Health) and **KKHNG**
...
length(kkh_ids)   # 27 registers: 8 from KKH, 19 from KKHNG
head(kkh_ids)
```
...
nrow(kkh) == length(unique(kkh$pnr))   # TRUE: one row per person, like any baseline cohort

# Mix a KKH id into a batch call with a real DST register, and join them --
...
`codebook(schema, "kkh_journal")` works too, with no extra code — it reads
the same `label`/`type` fields on any register, DST or KKH. So does
`label_columns()` -- worth doing for KKH/KKHNG especially, since a name
like `g03011` means nothing without one:

```{r kkh-label-columns}
kkh_labelled <- label_columns(kkh, schema, "kkh_journal")
attr(kkh_labelled$mdato, "label")
```
