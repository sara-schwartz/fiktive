...
`label_columns()` attaches that same label directly onto an already-generated
table's columns (the same `"label"` attribute convention `haven`/`labelled`
use, which RStudio's Data Viewer and `str()` pick up automatically) — useful
in its own right, but especially for DCH/DCH-NG, where a name like `g03011`
means nothing without one (see below):

```{r label-columns}
bef_labelled <- label_columns(tables$bef, schema, "bef")
attr(bef_labelled$koen, "label")
```

Column *names* never change — only this attribute is added. Rehearsed
analysis code has to run unmodified against the real register's own column
names later, so renaming stays something you'd do yourself, consistently,
on both fake and real data, if you do it at all; `label_columns()` adds
readability without that trade-off. Pass `lang = "da"` for the Danish label
instead of the English default.

## DCH and DCH-NG

Alongside the DST registers, `schema` always also includes fiktive's own
bundled metadata for **DCH** (Diet, Cancer and Health) and **DCH-NG**
(...and Next Generations) — variable name, type, and Danish/English label
only, per KKH/DCH's data-sharing approval. This is fiktive's own data, kept
separate from registers-guide (never fetched from or sent to that
project), and it's included automatically the moment you call
`load_registers_schema()` — not a second call to remember.
...
calculations, ...), not one file. fiktive collapses them into two
registers, `dch` and `dchng`, one row per person each — so you call
`generate_register()` once per cohort instead of once per real dataset —
but keeps the real per-dataset structure as a `dataset` field on every
column, surfaced by [codebook()]:

```{r dch-list}
dch_ids <- c("dch", "dchng")
cb <- codebook(schema, dch_ids)
length(unique(cb$dataset[cb$register == "dch"]))    # 8 real datasets merged into "dch"
length(unique(cb$dataset[cb$register == "dchng"]))   # 19 real datasets merged into "dchng"
subset(cb, register == "dch" & name %in% c("mdato", "vaegt", "g03001"))[, c("name", "dataset", "label_en")]
```

They generate exactly like any DST register — no separate function:

```{r dch-generate}
dch <- generate_register(
  "dch", pop, schema,
  from = as.Date("1993-12-01"), to = as.Date("1997-05-31"),
  seed = 1
)
nrow(dch) == length(unique(dch$pnr))   # TRUE: one row per person, like any baseline cohort

# Mix a DCH id into a batch call with a real DST register, and join them --
# both share the same population, so both join on pnr like any two fiktive tables
...
`codebook(schema, "dch")` works too, with no extra code — it reads the same
`label`/`type` fields on any register, DST or DCH. So does `label_columns()`
-- worth doing for DCH/DCH-NG especially, since a name like `g03011` means
nothing without one:

```{r dch-label-columns}
dch_labelled <- label_columns(dch, schema, "dch")
attr(dch_labelled$mdato, "label")
```

### Where DCH/DCH-NG's values come from
...
the cohort papers below, the rest (including all nutrient, food-group,
...
### A baseline cohort instead of repeated snapshots

`one_row_per = "person_reference_date"` above gives one row per person
**per snapshot date** — a 2-year annual window is 2 rows per person, not
one. If what you actually have is a baseline table — a cohort where each
person was measured (recruited, interviewed, sampled) exactly once,
possibly on a different date for each person, which is how real cohorts
staggered over months or years actually work — use `one_row_per = "person"`
instead:

```{r custom-register-person}
baseline <- generate_custom_register(
  id = "my_cohort",
  one_row_per = "person",   # exactly one row per eligible person, not a snapshot series
  columns = cols,
  population = pop,
  schema = schema,
  from = as.Date("2008-01-01"),   # recruitment window -- each person gets
  to = as.Date("2009-12-31"),     # one independent date somewhere in it
  seed = 1
  # no cadence= -- meaningless for this grain, errors if you set it
)

nrow(baseline) == length(unique(baseline$pnr))   # TRUE: exactly one row per person
```

Someone born after `to` is excluded entirely (never eligible), rather than
getting a row with a nonsensical date. If you declare `referencetid`
and/or `year` among your columns, they're filled from that person's own
drawn date, not `typed_noise()`.
...
id,integer,1,60000,,journal,Participant ID number,available,catalogue
...
ext <- generate_custom_register(
  ...
  columns = "mystudy_columns.csv",   # path to the CSV above
  population = pop,
  schema = schema,
